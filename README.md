...
`build_app.sh`는 기본적으로 버전 `2.2.0`, 빌드 `202608252025`(표시 `202608252025`)의 `MacSuperThumbnail/dist/NasFinder Super Thumbnail.app`을 arm64+x86_64 유니버설 바이너리로 만들고 ad-hoc 서명합니다. Developer ID 배포본은 `SIGN_IDENTITY`, `APP_VERSION`, `BUILD_NUMBER`, `DISPLAY_BUILD_NUMBER` 환경변수를 지정한 뒤 Apple 공증을 진행합니다.
